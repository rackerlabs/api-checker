...
if [ ! $PROG ] ; then { echo 'Missing $PROG environment variable. You should not call runcli.sh directly' ; exit 1; } fi

JAVA=$(which java)

command -v $JAVA >/dev/null 2>&1 || { echo $PROG ' requires java in the $PATH!'; exit 1; }
