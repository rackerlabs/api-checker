...
class MethodLabelHandlerTest extends BaseValidatorSuite with BeforeAndAfter with MockitoSugar {
  ...
  val xmlChecker =
    <checker xmlns="http://www.rackspace.com/repose/wadl/checker">
      ...
      <step id="POST-XML" type="METHOD" match="POST" next="REQ_TYPE_XML" label="post-xml-method"/>
      <step id="POST-JSON" type="METHOD" match="POST" next="REQ_TYPE_JSON" label="post-json-method"/>
      <step id="DELETE" type="METHOD" match="DELETE" next="Accept"/>
      <step id="REQ_TYPE_XML" type="REQ_TYPE" next="REQ_TYPE_XML_FAIL Accept" match="(?i)(application/xml)(;.*)?"/>
      <step id="REQ_TYPE_JSON" type="REQ_TYPE" next="REQ_TYPE_JSON_FAIL Accept" match="(?i)(application/json)(;.*)?"/>
