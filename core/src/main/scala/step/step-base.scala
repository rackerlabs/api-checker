...
abstract class Step(val id : String, val label : String) {
  ...
  //
  def check(req : CheckerServletRequest, resp : CheckerServletResponse, uriLevel : Int) : Option[CheckerResult]

  //
  //  Checks the step at the beginnig of the PATH
  //
  def check(req : CheckerServletRequest, resp : CheckerServletResponse) : Option[CheckerResult] = {
