...
/*
 *
 * Populates the response with the Result of api-checker if its an error.
 *
 */
class ServletResultHandler extends ResultHandler {
  ...
  def handle (req : CheckerServletRequest, resp : CheckerServletResponse, chain : FilterChain, result : Result)  : Unit = {
    if (!result.valid) {
      sendError(result, resp)
    }
  }

  private def sendError (er : Result, resp : CheckerServletResponse) : Unit = {
