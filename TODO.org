...
** TODO Header Enhancements [2/4]
   ...
   3. [-] Add support for rax:code and rax:message in Header checks [1/6]
      1. [X] Extend XSD to allow passing code, message to Header States
      2. [ ] Extend builder XSL to add code, message
      3. [ ] Make sure join optimizations are aware of the extension
      4. [ ] Modify Header states to accept code, message
      5. [ ] Modify handler to plug in state correctly
      6. [ ] TESTS [0/3]
         1. [ ] State
         2. [ ] Builder
         3. [ ] Validator
