...
** TODO Updates to XSD Schema[0/3]
   1. [ ] Checker should allow only one start node.
   2. [ ] Method must always have next nodes.
   3. [ ] Add grammar section (type, namespace, href)
   ...
   2. [-] Simple transation tests to confirm the lib is working [5/6]
      ...
      6. [ ] WADL Grammar[0/2]
         1. [ ] External Grammar[0/2]
            1. [ ] File Available, add to checker
            3. [ ] Missing File
         2. [ ] Inline Grammar(?)
...
** TODO URLXSD types [0/2]
   1. [ ] URLXSD node
   2. [ ] URLXSD/URL Fail nodes
