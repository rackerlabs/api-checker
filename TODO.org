...
** TODO Rax-Roles support
   *** DESIGN DECISIONS
   ...
   5. [ ] Test with rax-roles extension flags on
          ...
          7. [ ] no rax-roles on specified resource/method, rax-roles exist on a non-parent resource
         ...
         10. [ ] header roles is disabled, rax-roles is still processed
   ...
   6. [ ] Move rax-roles back into the builder.xsl
          ...
          3. [X] Only run the rax-roles xsl if the config is turned on
   7. [ ] Test with rax-roles extension flag off
          1. [X] Modify TestConfig to support flag for enable raxroles check
          2. [ ] Tests: when rax-roles is disabled, does not affect access
          3. [ ] rax-roles exist in wadl, header checks disabled, rax-roles are ignored
          4. [ ] rax-roles exist in wadl, header checks enabled, rax-roles are ignored
