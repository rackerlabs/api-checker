...
** TODO XSD Transform [5/9]
   ...
   8. [ ] Update filter-app
   9. [ ] Tests [0/2]
      1. [ ] Ensure that checker sets options correctly
      2. [ ] Make sure content is communicated upstream [0/4]
         1. [ ] Validate with transforms and options on XML (Xerces / Saxon)
         3. [ ] Validate with transforms and no options XML (Xerces / Saxon)
