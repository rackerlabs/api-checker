...
** TODO Rax-Roles support
   1. [ ] Update the config to account for the rax:roles flag
   2. [ ] Update the CLI to account for rax:roles (helpful in debugging during dev)
   3. [ ] Pass the flag to WADL->Checker format XSL
