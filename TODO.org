...
** TODO Checker to scala.[0/5]
   1. [ ] Build support for method/uri fail match in schema
   2. [ ] Populate method/uri fail match in checker conversion
   3. [ ] Update dot conversion to accunt for method/uri fail match
   4. [ ] Checker to scala as a handler -- possibly passing to an
      other handler
   5. [ ] Add support in validator
