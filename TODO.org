   ...
   2. [-] Simple transation tests to confirm the lib is working [4/5]
      ...
      5. [ ] WADL Grammar[0/2]
         1. [ ] External Grammar[0/2]
            ...
            2. [ ] Missing File
         2. [ ] Inline Grammar(?)
   3. [ ] Add Opt as an option, by pipeing through removeDups
   4. [-] Simple optimization test to confirm functionality[3/9]
        1. [X] OPT on but no duplicates
        2. [X] OPT on but no duplicates (tree)
        3. [X] OPT on but no duplicates (mixed)
        4. [ ] A single duplicate
        5. [ ] A single duplicate (tree)
        6. [ ] A single duplicate (mixed)
