...
** TODO Add support for schema XML schema check [4/9]
   ...
   5. [ ] Create schema parse pool if nessesary
   6. [ ] Transition other XSD checks to use pool
   7. [ ] Create XSD check state
   8. [ ] Add support for check state in the builder
   9. [ ] Tests
      1. [ ] States
      2. [ ] Validator
      3. [ ] Checker Tests
      ...
      5. [ ] WADL Validator
      6. [ ] Test on parameter check / include test for WELL_FORM to
      7. [ ] Test on grammar checks when no grammar is available.
